...
pub const BATCH_3D_ATTRIB_IDX: u32 = 0;
pub const BATCH_3D_ATTRIB_POS: u32 = 1;
pub const BATCH_3D_ATTRIB_COL: u32 = 2;
pub const BATCH_3D_ATTRIB_TEX: u32 = 3;
pub const BATCH_3D_ATTRIB_NOM: u32 = 4;
...
pub struct Batch3DProvider {
    ...
    normal_buffer: BatchBuffer<VertexBuffer>,
}

impl Batch3DProvider {
    pub fn new(render_bundle: &RenderBundle) -> Batch3D {
        let provider = Batch3DProvider {
            ...
            normal_buffer: create_vertex_batch_buffer(render_bundle),
        };
        Batch::new(provider)
    }
}
...
impl BatchProvider for Batch3DProvider {
    fn run(&mut self, command: BatchProviderCommand) {
        match &command {
            BatchProviderCommand::Add { id, data, .. } => {
                debug_assert_eq!(
                    data[BATCH_3D_ATTRIB_POS as usize].len(),
                    data[BATCH_3D_ATTRIB_COL as usize].len()
                );
                debug_assert_eq!(
                    data[BATCH_3D_ATTRIB_COL as usize].len(),
                    data[BATCH_3D_ATTRIB_TEX as usize].len()
                );
                debug_assert_eq!(
                    data[BATCH_3D_ATTRIB_TEX as usize].len(),
                    data[BATCH_3D_ATTRIB_NOM as usize].len()
                );

                self.index_buffer
                    .allocate(*id, data[BATCH_3D_ATTRIB_IDX as usize].len());
                self.position_buffer
                    .allocate(*id, data[BATCH_3D_ATTRIB_POS as usize].len() * 3);
                self.color_buffer
                    .allocate(*id, data[BATCH_3D_ATTRIB_COL as usize].len() * 4);
                self.texcoord_buffer
                    .allocate(*id, data[BATCH_3D_ATTRIB_TEX as usize].len() * 2);
                self.normal_buffer
                    .allocate(*id, data[BATCH_3D_ATTRIB_NOM as usize].len() * 3);
            }
            ...
            BatchProviderCommand::Replace {
                ...
                BATCH_3D_ATTRIB_IDX => self.index_buffer.reallocate(*id, data.len()),
                BATCH_3D_ATTRIB_POS => self.position_buffer.reallocate(*id, data.len() * 3),
                BATCH_3D_ATTRIB_COL => self.color_buffer.reallocate(*id, data.len() * 4),
                BATCH_3D_ATTRIB_TEX => self.texcoord_buffer.reallocate(*id, data.len() * 2),
                BATCH_3D_ATTRIB_NOM => self.normal_buffer.reallocate(*id, data.len() * 3),
                _ => {}
            },
            _ => {}
        }
    ...
    fn sort(&mut self, ids: Vec<u64>) -> HashSet<u32, RandomState> {
        ...
        set.insert(BATCH_3D_ATTRIB_IDX);
        set.insert(BATCH_3D_ATTRIB_POS);
        set.insert(BATCH_3D_ATTRIB_COL);
        set.insert(BATCH_3D_ATTRIB_TEX);
        set.insert(BATCH_3D_ATTRIB_NOM);
        set
    }
    ...
    fn flush(&mut self, batch_object_manager: &mut BatchObjectManager) {
        ...
        let mut normal_mapping = normal_buffer.buffer().open(0, normal_buffer.len());
        ...
        batch_object_manager.flush(|object, attribute| match attribute {
            BATCH_3D_ATTRIB_IDX => {
                ...
                object.for_each_v1u32(BATCH_3D_ATTRIB_IDX as usize, |i, v| {
                    index_mapping.set(v + p1.first as u32 / 3, i + p0.first);
                });
            }
            BATCH_3D_ATTRIB_POS => {
                let p1 = position_buffer.get_pointer(&object.id).unwrap();
                object.for_each_v3f32(BATCH_3D_ATTRIB_POS as usize, |i, v| {
                    position_mapping.set(v, i + p1.first);
                });
            }
            BATCH_3D_ATTRIB_COL => {
                let p2 = color_buffer.get_pointer(&object.id).unwrap();
                object.for_each_v4f32(BATCH_3D_ATTRIB_COL as usize, |i, v| {
                    color_mapping.set(v, i + p2.first);
                });
            }
            BATCH_3D_ATTRIB_TEX => {
                let p3 = texcoord_buffer.get_pointer(&object.id).unwrap();
                object.for_each_v2f32(BATCH_3D_ATTRIB_TEX as usize, |i, v| {
                    texcoord_mapping.set(v, i + p3.first);
                });
            }
            BATCH_3D_ATTRIB_NOM => {
                let p4 = normal_buffer.get_pointer(&object.id).unwrap();
                object.for_each_v3f32(BATCH_3D_ATTRIB_NOM as usize, |i, v| {
                    normal_mapping.set(v, i + p4.first);
                });
            }
            _ => {}
        });
        ...
        self.normal_buffer.buffer().close(normal_mapping);
    }
}
